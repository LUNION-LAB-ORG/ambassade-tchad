  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-4 md:p-6 w-full h-[340px] md:h-[500px] flex flex-col">
      <div className="flex items-center justify-between mb-2 md:mb-4">
        <div className="text-base md:text-xl font-bold text-gray-900 dark:text-white">{t('title')}</div>
        <div className="flex items-center gap-1 md:gap-2">
          <button onClick={() => setIndex(i => Math.max(0, i - 1))} className="w-7 h-7 md:w-8 md:h-8 flex items-center justify-center rounded bg-gray-50 text-gray-900 disabled:opacity-50"><ChevronLeft /></button>
          <button onClick={() => setIndex(i => Math.min(max, i + 1))} className="w-7 h-7 md:w-8 md:h-8 flex items-center justify-center rounded bg-orange-500 text-white"><ChevronRight /></button>
        </div>
      </div>
      <div className="flex gap-2 md:gap-4 overflow-x-auto flex-1 items-stretch">
        {news.slice(index, index + (isMobile ? 1 : 2)).map((item, idx) => (
          <div key={idx} className="min-w-[220px] md:min-w-[320px] max-w-xs bg-white dark:bg-gray-800 rounded-lg flex flex-col h-full">
            ...
            <div className="flex-1 flex flex-col justify-between">
              <div>
                ...
                <div className="font-bold text-gray-900 dark:text-white mb-1 text-sm md:text-base">{item.title}</div>
